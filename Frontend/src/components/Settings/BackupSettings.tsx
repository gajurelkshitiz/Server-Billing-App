...
const BackupSettings: React.FC<BackupSettingsProps> = ({ 
  ...
  const exportDatabase = async (format: 'json' | 'csv' | 'excel') => {
    setIsExporting(true);
    try {
      const response = await fetch(`${import.meta.env.REACT_APP_API_URL}/databaseExport/files?format=${format}`, {
        method: 'GET',
      ...
      switch (format) {
        case 'json':
          filename = `database-json-backup-${timestamp}.zip`;
          break;
        case 'csv':
          filename = `database-csv-backup-${timestamp}.zip`;
          break;
        case 'excel':
          filename = `database-excel-backup-${timestamp}.xlsx`;
          break;
      }
