      ...
      <div className="flex items-start justify-between">
        <div className="space-y-2 flex-1">
          ...
          <p className={`text-2xl font-bold ${colorClass} tracking-tight`}>
            ...
            Rs.{typeof amount === 'number' ? amount.toLocaleString('en-NP', { minimumFractionDigits: 2 }) : '-'}
          </p>
        </div>
...
const FinancialOverview: React.FC<FinancialOverviewProps> = ({ supplier }) => {
  // Get updateSupplierCreditLimit from hook
  const { updateSupplierCreditLimit, updatingCredit, creditError } = useSupplierData(undefined, supplier.id);
  const calculateBalance = () => {
    return supplier.prevClosingBalance + supplier.totalPurchase - supplier.totalPayment;
  };
  ...
  const [creditLimitAmount, setCreditLimitAmount] = useState<number | ''>(getPrimitiveNumber(supplier.creditLimitAmount));
  const [creditTimePeriodInDays, setCreditTimePeriodInDays] = useState<number | ''>(getPrimitiveNumber(supplier.creditTimePeriodInDays));

  const creditLimitExceeded =
    typeof supplier.creditLimitAmount === 'number' &&
    typeof supplier.totalDue === 'number' &&
    supplier.creditLimitAmount > 0 &&
    supplier.totalDue > supplier.creditLimitAmount;

  // Save handler using API
  const handleSave = async () => {
    if (typeof creditLimitAmount !== 'number' || typeof creditTimePeriodInDays !== 'number') return;
    const success = await updateSupplierCreditLimit(creditLimitAmount, creditTimePeriodInDays);
    if (success) setEditing(false);
  };

  console.log('Supplier in Financial Overview is: ', supplier);
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div>
                ...
                <div className="text-lg font-bold text-blue-700">
                  Rs.{getPrimitiveNumber(supplier.creditLimitAmount) !== '' ? Number(getPrimitiveNumber(supplier.creditLimitAmount)).toLocaleString('en-NP', { minimumFractionDigits: 2 }) : '-'}
                </div>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Credit Time Period (Days)</span>
                <div className="text-lg font-bold text-blue-700">
                  {getPrimitiveNumber(supplier.creditTimePeriodInDays) !== '' ? getPrimitiveNumber(supplier.creditTimePeriodInDays) : '-'}
                </div>
              </div>
              {creditLimitExceeded && (
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <FinancialCard
            ...
            amount={supplier.prevClosingBalance}
            icon={<DollarSign className="h-6 w-6" />}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <DateCard
            ...
            date={supplier.lastPurchaseDate}
            icon={<CalendarDays className="h-5 w-5" />}
            colorClass="text-blue-600"
          />
          <DateCard
            title="Last Payment Received"
            date={supplier.lastPaymentDate}
