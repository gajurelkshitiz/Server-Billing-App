...
const ComputerizedSalesEntryPage = () => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (editingSalesEntry) {
      ...
      const result = await addNewSalesEntryHandler();
      ...
      } else if (salesEntries.length > 0) {  // this will not get updated, so shows the previous code: its a bug as of now:
        // newId = salesEntries[0]._id; // fallback: show the latest
        console.log('Sales Entries: ', salesEntries);
        newId = salesEntries[salesEntries.length - 1]._id; // fallback: show the latest
      }
      setIsModalOpen(false);
      if (newId) {
        setPreviewSalesEntryId(newId);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-900">
          Sales Entry Management Computerized Version
        </h1>
        <button
...
export default ComputerizedSalesEntryPage;
