...
export function useSalesEntry() {
  ...
  const fetchSalesEntries = async (pageParam = page, limitParam = limit, filterParams = filters) => {
    setLoading(true);
    const baseUrl = `${import.meta.env.REACT_APP_API_URL}/computerizedSalesEntry`;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSalesEntries(data.computerizedSalesEntries || []);
  ...
  const addNewSalesEntryHandler = async () => {
    ...
    console.log("inside Add New Sales Entry Handler -> FormData is :- ", formData);
    ...
    try {
      ...
      console.log('result of response.json() just after calling the sales entry modal', res_data);
      if (response.ok) {
        toast({
          title: "Success",
        ...
        fetchSalesEntries();
        setFormData({});
        console.log("After Sales Entry, the new sales Entry Object is: ", res_data);
