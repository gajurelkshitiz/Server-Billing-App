...
const PayNowForm = ({ name, selectedCustomer, totalDue, totalAmount, onClose, onPaymentSuccess }) => {
  ...
  console.log('total Due left from summary.totalDueLeft', totalDue);


  const addNewCustomerPaymentHandler = async () => {
    const url = `${import.meta.env.REACT_APP_API_URL}/payment/customer`;
    try {
      ...
      if (response.ok) {
        toast({
          title: "Success",
          description: `${selectedCustomer.name} Payment was Successfully.`,
        });
        // fetchFiscalYears();
