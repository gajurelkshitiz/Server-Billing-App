...
const createComputerizedPurchaseEntry = async (req, res) => {
  ...
  console.log('Inside purchase Entry : Items ==> ', items);
  ...
  // Get customer name
  const supplierObj = await Supplier.findById(supplierID);
  if (!supplierObj) {
  ...
  let lastEntry = await ComputerizedPurchaseEntry.findOne({ companyID, fiscalYear })
    .sort({ createdAt: -1 })
    .select("invoiceNo")
    .lean();
  ...
  const computerizedPurchaseEntry = await ComputerizedPurchaseEntry.create({
    supplierID,
    supplierName,
    invoiceNo,
    date,
  ...
  res.status(StatusCodes.CREATED).json({ computerizedPurchaseEntry });
};

// Get all computerized sales entries
const getAllComputerizedPurchaseEntries = async (req, res) => {
  ...
  // Count & query
  const total = await ComputerizedPurchaseEntry.countDocuments(filter);
  const computerizedPurchaseEntries = await ComputerizedPurchaseEntry.find(filter)
    .sort('createdAt')
    .skip(startIndex)
    .limit(limit);
  ...
  res.status(StatusCodes.OK).json({
    ...
    computerizedPurchaseEntries,
    count: computerizedPurchaseEntries.length,
  });
};

// Get a single computerized sales entry
const getComputerizedPurchaseEntry = async (req, res) => {
  const {
    params: { id: computerizedPurchaseEntryID },
  } = req;

  const computerizedPurchaseEntry = await ComputerizedPurchaseEntry.findOne({ _id: computerizedPurchaseEntryID });

  if (!computerizedPurchaseEntry) {
    throw new notFoundError(`No Computerized Purchase Entry Found.`);
  }

  res.status(StatusCodes.OK).json({ computerizedPurchaseEntry });
};
